...
> 💡 События можно отправлять даже до полной загрузки каталога, однако для корректного обогащения признаками лучше сначала импортировать основные SKU (или выполнять синхронизацию не реже одного раза в сутки), иначе события с «неизвестными» товарами попадут в очередь без продуктовых метаданных.
